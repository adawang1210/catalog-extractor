...
- [ ] **OSR-1|孤兒磁磚圖召回(草案送審中=output/orphan_swatch_recall_design.md)**:
      使用者追問「為什麼連磁磚也沒被標」→ 病灶=有碼廠的「偵測到但配不到碼」磁磚圖被
      當非產品丟棄(pipeline.py:578 只有零綁定檔才輸出無碼色樣)。使用者提案訊號=
      「圖旁有尺寸→高機率真磁磚」;加 E-1 面積閘擋場景照。**分離量測**(dev/orphan_swatch_probe.py,
      通案三入庫、selftest 綠、數字可復現):全 14 檔孤兒圖 33,497→篩出 1,899、大面積(疑場景)
      擋掉 469;Emil Be-Square 125→14(p16 左欄 6 真磁磚全在內=召回成立)、面積閘有效
      分離場景照(全池 469)。**★但書**:篩出 1,899 中 1,840(97%)全來自 Topcer 拼磚一檔
      (馬賽克小格誤篩),正規化須排除塌縮/拼磚頁(沿用既有 assembly_collapse 偵測);
      扣 Topcer 其餘 13 檔僅 59 張、合理。**與②型認碼死路分野**:本案=呈現層可修(不碰偵測/綁定,
      只顯示已偵測到的磁磚圖、標「磁磚·碼未知」進佇列、不成 Variant)、零過擬合(純 per-doc 幾何)。
      **送審前必辦**=全池偽陽性率人工抽核(篩出集裡真磁磚 vs 帶尺寸裝飾圖/圖例);正規化=
      reason=orphan_swatch 佇列項+前端新框類+紅燈先行+smoke 守恆+拼磚排除 diff。零規則改動承諾照舊。
